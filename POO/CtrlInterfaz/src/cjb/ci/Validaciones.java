...
public class Validaciones
{
    ...
    public static LocalDate validaFecha(JFrame jf, int len, String s, LocalDate fecha, JTextField cj)
    {
        // Define una expresión regular para el formato de fecha (dd/mm/aaaa)
        ...
        // Comprueba si la cadena coincide con el patrón y si los valores de día, mes y año son válidos
        if (matcher.matches())
        {
            ...
            // Verifica que el día, el mes y el año sean válidos
            if (dia >= 1 && dia <= 31 && mes >= 1 && mes <= 12 && año >= 1900 && año <= 9999)
            {
//                CtrlInterfaz.habilita(true,cj2);
//                CtrlInterfaz.cambia(cj2);
            } else
            {
                Mensaje.error(jf, "Fecha incorrecta, ingresa una fecha con formato (dd/mm/aaaa)");
                CtrlInterfaz.limpia(cj);
//                CtrlInterfaz.habilita(false, cj,cj2);
                CtrlInterfaz.cambia(cj);
            }
        }
        // Si la longitud de la cadena no es igual a 10 (dd/mm/aaaa), devuelve falso
        if (len != 10)
        {
            Mensaje.error(jf, "Fecha incorrecta, ingresa una fecha con formato (dd/mm/aaaa)");
            CtrlInterfaz.limpia(cj);
            CtrlInterfaz.habilita(false, cj);
            CtrlInterfaz.cambia(cj);
        }
        return fecha;
    }
    ...
    public static void enterCadenaNoVacia(JFrame jf, KeyEvent ke, JTextArea cj, Object obj)
    {
        if (ke.getKeyChar() == '\n')
        {
            if (cj.getText().length() != 0)
